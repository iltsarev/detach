...
typed helper XPC method. The helper persists it in helper state. A missing, unknown, or wrong-type
stored value becomes 10% without discarding otherwise valid leases or
ownership. Any other write is rejected. `detach config`
