...
Settings → Updates can enable automatic download only while Sparkle
`allowsAutomaticUpdates` is true. The current bundle does not set
`SUAllowsAutomaticUpdates`, so this follows automatic checks. The default
is off. Sparkle then downloads a signed update in the background and
installs the app when Detach quits. Authorization can still require
confirmation. Do not set `SUAutomaticallyUpdate` in Info.plist.
