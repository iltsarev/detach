...
enum PetGenerationPhase: Equatable {
    ...
    static func resolve(
        ...
        hasFreshSessionSnapshot: Bool = true,
        trackingTimedOut: Bool = false
    ) -> PetGenerationPhase {
        if isStarting { return .starting }
        if !pendingPetID.isEmpty && !pendingSessionID.isEmpty {
            if trackingTimedOut { return .trackingTimedOut }
            guard let pendingSessionStatus else {
                return hasFreshSessionSnapshot
                    ? .missingSession : .checkingSession
            }
...
enum PetGenerationAvailability: Equatable {
    case available
    case missingSkill
    case missingHelper
    case missingRuntime
...
enum PetGenerationSupport {
    ...
    static func isAvailable(
        ...
        availability(
            ...
            fileManager: fileManager).isAvailable
    }
    ...
    static func availability(
        ...
        guard fileManager.isExecutableFile(atPath: runtimeHelperURL.path) else {
            return .missingHelper
        }
        guard WorkspaceDependenciesMCPServer.runtimeIsAvailable(
            at: runtimeRoot,
...
enum PetGenerationSessionMonitor {
    static let packageCheckIntervalNanoseconds: UInt64 = 2_000_000_000
    static let maximumPackageChecks = 1_800
