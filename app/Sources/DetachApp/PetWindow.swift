...
final class PetWindowController: NSObject, ObservableObject, NSWindowDelegate {
    ...
    func configure(
        ...
        openPetSettings: @escaping () -> Void,
        openMainWindow: @escaping () -> Void
    ) {
        self.coordinator = coordinator
        coordinator.onVisibilityChange = { [weak self] in
    ...
    private func makePanel(
        ...
        onOpenPetSettings: @escaping () -> Void
    ) -> NSPanel {
        let panel = PetPanel(
...
enum PetSessionNavigator {
    static func open(
        ...
        if let activity {
            navigation.requestSession(activity.sessionID)
        } else {
...
enum PetWindowContextAction: Equatable {
    case openSettings
    case hide
}

@MainActor
enum PetWindowContextActionHandler {
    static func perform(
        _ action: PetWindowContextAction,
        coordinator: PetCoordinator,
...
private struct PetFloatingView: View {
    ...
    var body: some View {
        ...
        .contextMenu {
            Button {
                performContextAction(.openSettings)
            } label: {
                Label(
                    L10n.string("Pet Settings…"),
                    systemImage: "gearshape")
            }
            Button {
                performContextAction(.hide)
            } label: {
                Label(L10n.string("Hide Pet"), systemImage: "eye.slash")
            }
        }
    ...
    private var animationIdentity: String {
        ...
        return "\(activity.lifecycleID):\(activity.state.rawValue)"
    }
    ...
    private func performContextAction(_ action: PetWindowContextAction) {
        PetWindowContextActionHandler.perform(
            action,
            coordinator: coordinator,
            openPetSettings: onOpenPetSettings)
    }
}
