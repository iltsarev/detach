...
"$SCRIPT" --name integration --detach -- "$literal_prompt"
...
"$SCRIPT" logs integration | grep -F 'fake Codex finished' >/dev/null
...
"$SCRIPT" recover --detach integration
sleep 1
...
"$SCRIPT" --name integration --detach -- 'start a new thread'
sleep 1
...
if "$SCRIPT" --name integration --detach -- 'must not replace a running task'; then
...
export FAKE_CODEX_INIT_DELAY=5
"$SCRIPT" --name integration --detach -- 'replace the completed thread'
sleep 1
